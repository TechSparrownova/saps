...
from frappe.utils import cstr, get_snova_path, is_html, strip, strip_html_tags, unique
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	for app in apps or frappe.get_installed_apps(_ensure_on_snova=True):
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else frappe.get_installed_apps(_ensure_on_snova=True):
