...
def _new_site(
	...
	with filelock("snova_new_site", timeout=1):
		install_db(
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/sparrownova/snova/blob/develop/snova/utils/__init__.py#L390
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/sparrownova/snova/blob/develop/snova/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/sparrownova/snova/blob/develop/snova/app.py#L114
...
def extract_sql_from_archive(sql_file_path):
	...
	SQL file or an SQL file path. The path may be absolute or relative from the snova
	...
	"""
	from frappe.utils import get_snova_relative_path
...
def is_downgrade(sql_file_path, verbose=False):
	"""checks if input db backup will get downgraded on current snova"""
