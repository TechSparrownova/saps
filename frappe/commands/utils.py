...
def build(
	...
	with filelock("snova_build", is_global=True, timeout=10):
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		if not (allow_tests or os.environ.get("CI")):
			...
			click.secho(f"snova --site {site} set-config allow_tests true", fg="green")
			return
...
@click.option(
	"-g", "--global", "global_", is_flag=True, default=False, help="Set value in snova config"
)
