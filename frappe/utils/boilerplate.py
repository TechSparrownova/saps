...
def _create_app_boilerplate(dest, hooks, no_git=False):
	...
	# this is needed because if public doesn't exist, snova build doesn't symlink the apps assets
...
dynamic = ["version"]
dependencies = [
    # "frappe~=15.0.0" # Installed and managed by snova.
]
    ...
    steps:
      ...
      - name: Setup
        run: |
          ...
          snova init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/frappe-snova
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/frappe-snova
        run: |
          snova get-app {app_name} $GITHUB_WORKSPACE
          snova setup requirements --dev
          snova new-site --db-root-password root --admin-password admin test_site
          snova --site test_site install-app {app_name}
          snova build
        env:
          CI: 'Yes'
      ...
      - name: Run Tests
        working-directory: /home/runner/frappe-snova
        run: |
          snova --site test_site set-config allow_tests true
          snova --site test_site run-tests --app {app_name}
        env:
          TYPE: server
"""
