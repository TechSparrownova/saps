...
def get_snova_path():
	return os.environ.get("FRAPPE_SNOVA_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(frappe.__file__), "..", "..", "..")
	)


def get_snova_id():
	return frappe.get_conf().get("snova_id", get_snova_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or frappe.local.site}@{get_snova_id()}"
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                snova --site erpnext.local execute frappe.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
...
def call(fn, *args, **kwargs):
	...
	Example:
	        via terminal:
	                snova --site erpnext.local execute frappe.utils.call --args '''["frappe.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
