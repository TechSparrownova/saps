...
from frappe.utils import add_to_date, get_snova_path, get_snova_relative_path, now
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("snova --site {site} execute frappe.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("snova --site {site} execute frappe.local.site")
		...
		self.execute("""snova --site {site} execute frappe.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
	...
	def test_restore(self):
		...
		self.execute("snova --site {test_site} backup --ignore-backup-conf --with-files", site_data)
		self.execute(
			"snova --site {test_site} execute frappe.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		self.execute(
			"snova --site {test_site} restore {database} --with-public-files {public} --with-private-files {private} ",
			site_data,
		)
		self.assertEqual(self.returncode, 0)
		...
		self.execute(
			"snova --site {test_site} execute" " frappe.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_frappe_site_env(self):
		...
		self.execute("snova execute frappe.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_snova_drop_site_should_archive_site(self):
		...
		self.execute(f"snova drop-site {site} --force --root-password {frappe.conf.root_password}")
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		self.execute(f"snova --site {TEST_SITE} set-config admin_password {frappe.conf.admin_password}")
...
class TestCommandUtils(FrappeTestCase):
	def test_snova_helper(self):
		from frappe.utils.snova_helper import get_app_groups
...
class TestDBCli(BaseTestCommands):
	...
	def test_db_cli_with_sql(self):
		self.execute("snova --site {site} db-console -e 'select 1'")
		self.assertEqual(self.returncode, 0)
